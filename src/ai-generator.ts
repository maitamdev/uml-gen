...
function cleanMermaidCode(raw: string): string {
  ...
  // Search for ANY second diagram keyword starting after the first keyword's declaration
  const firstKeywordEnd = code.indexOf(' ') > 0 ? code.indexOf(' ') : code.indexOf('\n');
  if (firstKeywordEnd > 0) {
    for (const kw of diagramKeywords) {
      const secondIdx = code.indexOf(kw, firstKeywordEnd);
      if (secondIdx > 0) {
        code = code.substring(0, secondIdx).trim();
        break;
      }
    }
  }
